...
def find_latest_csv(directory):
    latest_file = None
    latest_time = -1
    csv_pattern = re.compile(r'p_time=(\d+)\.csv')
...
def process_all_directories(root_dir):
    dest_path = os.path.join(root_dir, "p_Time=99")
...
# 実行するディレクトリを指定
results_directory = "/mnt/inlet_value2/inlet_size12_120000_test"
if not os.path.exists(f"{results_directory}/p_Time=99"):
    os.mkdir(f"{results_directory}/p_Time=99")
