...
if learning == 1:
    ...
    #sys.exit()
    train_x = np.reshape(train_x,[len(train_x),2,-1])
    train_y = np.reshape(train_y,[len(train_y),2,-1])
    ...
    test_x = np.reshape(test_x,[len(test_x),2,-1])
...
evaluate_x = np.reshape(evaluate_x,[len(evaluate_x),2,-1])
evaluate_y = np.reshape(evaluate_y,[len(evaluate_y),2,-1])
