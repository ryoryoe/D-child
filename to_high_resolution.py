...
from preprocess import Preprocessing_standard_mix_for, Preprocessing_high_resolution, Preprocessing_high_resolution_p
...
def ddpm_train(params):
    ...
    if params.learning ==1: 
        ...
        #trainデータの前処理
        train_x,train_y,file_names,_,_,vx,vy,inlet_left_list,inlet_right_list,outlet_left_list,outlet_right_list, train_p,evals_p,file_names_p = Preprocessing_high_resolution_p(train_path,train_eval_path,params.width,params.standard,params.cut,mode="train")
        #train_x,train_y,file_names,_,_,vx,vy,inlet_left_list,inlet_right_list,outlet_left_list,outlet_right_list = Preprocessing_high_resolution(train_path,train_eval_path,params.width,params.standard,params.cut,mode="train")
        print(f"{len(train_x)=}")
        print(f"{len(train_y)=}")
        ...
        train_p = torch.tensor(train_p, dtype=torch.float32)
        evals_p = torch.tensor(evals_p, dtype=torch.float32)
        ...
        train_p,evals_p = train_p[:params.cut_size],evals_p[:params.cut_size]
        trainset = torch.utils.data.TensorDataset(train_x,train_y,vx,vy,inlet_left_list,inlet_right_list,outlet_left_list,outlet_right_list,train_p,evals_p)
        #trainset = torch.utils.data.TensorDataset(train_x,train_y)
        dataloader = torch.utils.data.DataLoader(trainset,batch_size = params.batch_size, num_workers = 2, drop_last=True)

        #testデータの前処理
        #test_x,test_y,file_names_test,avg_list,std_list,vx_test,vy_test,inlet_left_list_test,inlet_right_list_test,outlet_left_list_test,outlet_right_list_test = Preprocessing_high_resolution(estimate_path,estimate_eval_path,params.width,params.standard,params.cut,mode="test")
        test_x,test_y,file_names_test,avg_list,std_list,vx_test,vy_test,inlet_left_list_test,inlet_right_list_test,outlet_left_list_test,outlet_right_list_test,test_p,evals_p,file_names_p = Preprocessing_high_resolution_p(estimate_path,estimate_eval_path,params.width,params.standard,params.cut,mode="test")
        test_x = torch.tensor(test_x, dtype=torch.float32)
        test_y = torch.tensor(test_y, dtype=torch.float32)
        vx_test = torch.tensor(vx_test, dtype=torch.float32)
        vy_test = torch.tensor(vy_test, dtype=torch.float32)
        test_p = torch.tensor(test_p, dtype=torch.float32)
        evals_p = torch.tensor(evals_p, dtype=torch.float32)
        ...
        test_p = torch.tensor(test_p, dtype=torch.float32)
        evals_p = torch.tensor(evals_p, dtype=torch.float32)
        testset = torch.utils.data.TensorDataset(test_x,test_y,vx_test,vy_test,inlet_left_list_test,inlet_right_list_test,outlet_left_list_test,outlet_right_list_test,test_p,evals_p)
    ...
    #eval_x,eval_y,file_names_estimate,avg_list,std_list,vx_eval,vy_eval,inlet_left_list_eval,inlet_right_list_eval,outlet_left_list_eval,outlet_right_list_eval = Preprocessing_high_resolution(estimate_path,estimate_eval_path,params.width,params.standard,params.cut,mode="eval")
    eval_x,eval_y,file_names_estimate,avg_list,std_list,vx_eval,vy_eval,inlet_left_list_eval,inlet_right_list_eval,outlet_left_list_eval,outlet_right_list_eval,evals_p,evals_evals_p,file_names_p = Preprocessing_high_resolution_p(estimate_path,estimate_eval_path,params.width,params.standard,params.cut,mode="eval")
    ...
    evals_p = torch.tensor(evals_p, dtype=torch.float32)
    evals_evals_p = torch.tensor(evals_evals_p, dtype=torch.float32)
    evalset = torch.utils.data.TensorDataset(eval_x,eval_x,vx_eval,vy_eval,inlet_left_list_eval,inlet_right_list_eval,outlet_left_list_eval,outlet_right_list_eval,evals_p,evals_evals_p)
    ...
    if params.learning == 1:
        ...
        #modelの定義
        model = modules.To_high_resolution_with_inlet_value(UNet=UNet_high_resolution()).to(device)
        ...
        for epoch in epoch_bar:
            ...
            for iter, (x,y,vx,vy,inlet_l,inlet_r,outlet_l,outlet_r,train_p,evals_p) in enumerate(dataloader):
                x = x.to(device)
                y = y.to(device)
                vx,vy = vx.to(device),vy.to(device)
                vx = vx.unsqueeze(1)
                vy = vy.unsqueeze(1)
                evals_p = evals_p.to(device)
                ...
                predict_v = out[0]
                predict_p = out[1]
                #loss,losses = loss_fn(out,y,v_loss,evals_p)
                loss,losses = loss_fn(predict_v,predict_p,y,evals_p,v_loss)
                if loss.isnan():
                    print("loss is nan")
                    print(f"{v=}")
                    sys.exit()
            ...
            with torch.no_grad():  # テスト時は勾配計算を行わない
                ...
                for iter, (x,y,vx,vy,inlet_l,inlet_r,outlet_l,outlet_r,train_p,evals_p) in enumerate(test_loader):
                    x = x.to(device)
                    y = y.to(device)
                    vx,vy = vx.to(device),vy.to(device)
                    vx = vx.unsqueeze(1)
    ...
    for counter,(data,evaly,vx_eval,vy_eval,inlet_l_eval,inlet_r_eval,outlet_l_eval,outlet_r_eval,evals_p,evals_evals_p) in enumerate(estimate_loader):
...
class HyperParameters:
    ...
    output_path: str = "inlet_value2_1126_with_PINNs_NS"
    #output_path: str = "inlet_value1_1003"
    message: str = "16*16に入り口風速も加えて32*32を当てに行く。NS方程式を加える"
    file_path: str = "inlet_value2_test" #推定に使うデータのフォルダ
    train_file_path = "inlet_value2" #学習データのフォルダ
    train_path: str = f"../{train_file_path}" #学習データ
    train_eval_path: str  = f"../{train_file_path}" #学習データの正解ラベル
    test_path: str = f"../{file_path}" #推定に使うデータ
    test_eval_path: str  = f"../{file_path}" #推定に使うデータ(意味ない)
 ...
 #画像の幅
    ...
    target_epoch: int = 750 #どのエポックのモデルを使って推定するか
    #target_epoch: int = 2000 #どのエポックのモデルを使って推定するか
    ...
    additional_epoch = 1001 #学習を再開するエポック数(重みファイルのepoch+1)
