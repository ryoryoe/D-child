...
class CFDVelocityDataset(Dataset):
    def __init__(self, base_dir, chunk_size=5000, max_value=1e3, test=False):
        ...
        self.time_dirs = [os.path.join(base_dir, f"Time={t}") for t in range(1, 6)]
    ...
    def _process_file_group(self, f1):
        ...
        if (os.path.exists(pattern_t2) and
            ...
            if any(arr is None for arr in [arr1, arr2, arr3, arr4, arr5]):
                # Skip processing if any file contains NaN or invalid values
                return None

            input_data = np.stack([arr1, arr2, arr3, arr4], axis=1)  # (2, 4, 32, 32)
            label_data = arr5[:, :, :]  # (2, 1, 32, 32)
            label_filename = os.path.basename(pattern_t5)
