...
def main():
    ...
    H, data_train, data_valid_or_test, preprocess_fn = set_up_data(H) #ここで前処理を終えたデータを取得
    vae, ema_vae = load_vaes(H, logprint) #load_vaesでvaeとema_vaeを取得(vae.pyにvaeのモデルがある)
    if H.test_eval:
