...
def save_checkpoint(model,loss_weights,  optimizer, epoch, loss, filename='checkpoint.pth'):#重みを保存
    checkpoint = {
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'loss_weights_state_dict': loss_weights.state_dict(),  # 追加
        'loss': loss
    }
    torch.save(checkpoint, filename)
 
def load_checkpoint(model,loss_weights,  optimizer, filename='checkpoint.pth'): #重みを読み込む
    print(f"load {filename}")
    checkpoint = torch.load(filename)
    model.load_state_dict(checkpoint['model_state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    loss_weights.load_state_dict(checkpoint['loss_weights_state_dict'])
    epoch = checkpoint['epoch']
    loss = checkpoint['loss']
    data_loss_weight, physics_loss_weight, wall_loss_weight, inlet_loss_weight = loss_weights.get_weights()
    print(f"Data Loss Weight: {data_loss_weight.item():.4f}, " +
          f"Physics Loss Weight: {physics_loss_weight.item():.4f}, " +
          f"Wall Loss Weight: {wall_loss_weight.item():.4f}, " +
          f"Inlet Loss Weight: {inlet_loss_weight.item():.4f}")
    return model,loss_weights,  optimizer, epoch, loss
...
def ddpm_train(params):
    ...
    if params.learning == 1:
        ...
        if params.additional_learning: 
            model,loss_weights, optimizer, start_epoch, loss = load_checkpoint(model, loss_weights, optimizer, f"{params.root_path}/result/{params.output_path}/save_temp_weight/checkpoint_{params.output_path}_epoch={params.additional_epoch-1}.pth")
                # エポックの終了時に損失関数の重みを表示
        else:
            file_maker(f"{params.root_path}/result/{params.output_path}")
            condition_text(params.message,params.output_path,params)
        ...
        for epoch in epoch_bar:
            ...
            for iter, (x,y,vx,vy,inlet_l,inlet_r,outlet_l,outlet_r) in enumerate(dataloader):
                ...
                out = model(v)
                loss,losses = loss_fn(out,y,v_loss,loss_weights)
            ...
            if epoch % params.save_interval == 0 and epoch != 0:    
                ...
                save_checkpoint(model,loss_weights, optimizer, epoch, loss, f"{params.root_path}/result/{params.output_path}/save_temp_weight/checkpoint_{params.output_path}_epoch={epoch}.pth")
...
class HyperParameters:
    ...
    output_path: str = "pinns_estimate_1117"
    message: str = "PINNsによる速度推定。重みも学習させてみる"
