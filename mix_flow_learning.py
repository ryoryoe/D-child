...
def ddpm_train(params):
    ...
    if params.learning == 1:
        ...
        #model = modules.Input_Mix_divide_skip_detailed_12(UNet=UNet_low()).to(device)
        #model = modules.Input_Mix_divide_skip_detailed_12(UNet=UNet_lower()).to(device)
        #model = modules.Input_Mix_divide_skip_detailed_12(UNet=UNet_deepest()).to(device)
...
class HyperParameters:
    ...
    output_path: str = "inlet_value1_1224"
    #output_path: str = "inlet_value1_1202"
    message: str = "位置エンコーディングをフーリエに変更"
    file_path: str = "mix_flow2" #推定に使うデータのフォルダ
    train_file_path = "mix_flow2" #学習データのフォルダ
