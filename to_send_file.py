...
model = torch.load(weight_eval_path,map_location=torch.device('cpu'))
model = model.to(device)
model.eval()
label_list = []
pred_list = []
...
for i in range(len(output_list)):
    out = output_list[i]
    out = out.T
    pd.DataFrame(out,columns=["X Velocity","Y Velocity"]).to_csv(f"estimate_{file_names[i]}.csv", index=False)
