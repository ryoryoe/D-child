...
from modules_ver2 import UNet,Encoder,Decoder,Stable_diffusion
...
def ddpm_train(params):
    ...
    if params.learning ==1: 
        ...
        train_x,train_y,file_names,_,_= Preprocessing_standard(train_path,train_eval_path,params.width)
        train_x = torch.tensor(train_x, dtype=torch.float32)
        train_y = torch.tensor(train_y, dtype=torch.float32)
        a = train_x[:3350]
        b = train_x[:3400]
        train_x = torch.cat([a,b])
        #train_x = train_x[3100:3500]
        a = train_y[:3350]
        b = train_y[:3400]
    ...
    eval_x,eval_y,file_names_estimate,avg_list,std_list = Preprocessing_standard(estimate_path,estimate_eval_path,params.width)
    ...
    model = Stable_diffusion().to(device)
    ...
    if params.learning == 1:
        ...
        def loss_function(label, predict, mu, log_var):
            ...
            reconstruction_loss = loss_fn(predict, label)
        ...
        for epoch in epoch_bar:
            ...
            for iter, (x,y) in enumerate(dataloader):
                ...
                optimizer.zero_grad()
                out,encode_avg,encode_std = model(x)#modelに一度通せばノイズを取り除いた画像が出てくるので正解との誤差を計算できる
                #out = decoder(out)
                loss = loss_function(y,out,encode_avg,encode_std)#ノイズと予測したノイズの誤差を計算
    ...
    for counter,(data,_) in enumerate(estimate_loader):
            ...
            #x = torch.stack((encode_avg, encode_std), dim=1)
            #xt, t, noise = ddpm.diffusion_process(x)
            out,_,_ = model(data)#modelに一度通せばノイズを取り除いた画像が出てくるので正解との誤差を計算できる
...
class HyperParameters:
    ...
    output_path: str = "vae_diffusion_model_0221_T=20_from5_to20"
    #file_path: str = "test_mesh32"
    file_path: str = "train_data_ver6_test"
    #file_path: str = "train_data_ver7"
    train_path: str = "../train_data_ver10/Time=5"
    train_eval_path: str  = "../train_data_ver10/Time=20"
    #train_path: str = "../test_mesh32/Time=1"
    #train_eval_path: str  = "../test_mesh32/Time=20"
